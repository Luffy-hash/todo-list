...
class DetailMeteo extends StatefulWidget {
  final int id;
  const DetailMeteo({super.key, required this.id});
...
class _DetailMeteoState extends State<DetailMeteo> {
  ...
  LatLng _coords = const LatLng(46.6061, 1.8752);
  ...
  Future<void> _getLatLong(String address) async {
    const api = "AIzaSyDhYXaBrAlrKAUj_Mjbyvc4bAPebVFIy3A";
    final url =
        ...
        if (data['status'] != "ZERO_RESULTS" && !_changed) {
          setState(() {
            ...
            mapController.animateCamera(CameraUpdate.newCameraPosition(
                CameraPosition(target: _coords, zoom: 15)));
            location.add(Marker(
                markerId: const MarkerId("location"), position: _coords));
          });
        }
      }
    }
                        ...
                        if ((snapshot.data?.city == null ||
                          ...
                          return const SizedBox(
                            child: Column(
                              ...
                              children: [
                                Text(
                                    "Vous n'avez pas renseigné d'adresse ou adresse invalide",
                                    style: TextStyle(
                                        fontSize: 22,
                          ...
                          return SizedBox(
                            child: Column(
                              ...
                              children: [
                                Text(
                                    "${snapshot.data?.streetnumber} ${snapshot.data?.street}, ${snapshot.data?.codePostal} ${snapshot.data?.city}",
